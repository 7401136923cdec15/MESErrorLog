package com.mes.errorlog.server.shristool;
...
import com.mes.errorlog.server.service.utils.XmlTool;
...
public class LoggerTool {
	...
	public void Start() {
		...
		Init_LoggerPath("ELGService", "elglogs", 15, 1, 1);
		...
		new Thread(() -> {
			...
			while (true) {
				try {
					...
					if (wTicks % 10 == 0) {
						LoggerTool.SaveLoggerFunctionList("iplantmes");
						LoggerTool.SaveApplicationFaultList("iplantmes");
					}
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					LoggerTool.SaveException("iplantmes", "MainThread", "Excetion:" + e.toString());
				}
			}
	...
	public static synchronized boolean SaveException(String wModuleName, String wFunctionName, String wBugInfo) {
		try {
			...
			if (wSessionTime.LoggerEnable) {
				...
				// ==============创建保存文件==============
				BufferedWriter wOutFile = new BufferedWriter(
						new OutputStreamWriter(new FileOutputStream(wPath_File), "GBK"));
				wOutFile.write(wMessageText);
				wOutFile.newLine();
				wOutFile.flush();
				wOutFile.close();
			}
	...
	public static synchronized boolean Clear_ExpiredLoggerFile(int wExpiredDays) {
		try {
			...
			wCalendarE.add(Calendar.DATE, -wExpiredDays);
			int wYearTime = wCalendarC.get(Calendar.YEAR);
			...
			Calendar wExpiredDate = Calendar.getInstance();
			// ===异常系统日志文件=============
			String wExceptionPath_Log = LoggerTool.LoggerPath + "Exception";
			...
			// ===系统日志文件=============
			String wLoggerPath_Log = LoggerTool.LoggerPath + "Logger";
